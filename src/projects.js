 ...
 *
 *   screenshot    path to a 16:10 image in assets/. Absent → tasteful placeholder
 ...
 * URLs marked  // [ASK ME]  are placeholders — replace with real links.
 * =========================================================================== */
...
export const projects = [
  {
    ...
    product: {
      problem:
        "Support reps burn minutes every ticket hunting for the right saved reply, SOP, or doc — context-switching out of the conversation to go find the thing they already know exists.",
      decision:
      ...
      outcome:
        "A local-first tool reps actually keep open — resources are one keystroke away instead of a tab-hunt. Released publicly with the product thinking documented, not just the code."
    },
    tech: {
      arch:
...

  {
    ...
    detail: {
      body:
        "The insight here was economic, not editorial: every repeat ticket is a doc that doesn't exist yet. So I instrumented which questions actually recurred, then built a pipeline that turned that signal into publishable articles at scale.\n\nThe hard part wasn't drafting — it was consistency. A fixed framework (every article the same shape, same required sections, same voice) meant AI drafting could be fast without going off the rails, and a human review gate kept accuracy honest. The result reads like one author wrote all 700+ pieces.",
      gallery: [],
      metrics: [
...

  {
    ...
    product: {
      ...
      decision:
        "Aim for consistency at scale: draft replies in the established house voice from the team's own ticket history, with a human always in the loop to approve and adjust before anything sent.",
      outcome:
        "Faster, more consistent first drafts — reps edit instead of writing from scratch, and the brand voice holds steady across the team."
    },
    tech: {
      arch:
        "RAG over historical Zoho Desk tickets: embed past resolved tickets, retrieve the closest precedents for an incoming issue, and condition the draft on both the retrieved context and the house-voice guidelines. Exposed to tooling via MCP.",
      stack: ["RAG", "Embeddings", "MCP", "Zoho Desk data", "LLM"],
      calls:
        "Grounded generation over a raw prompt — retrieval from real past tickets keeps drafts accurate and on-voice instead of plausible-but-wrong. Kept humans in the loop by design; the assistant drafts, it never sends."
    },

    detail: {
      body:
        "This one is about consistency as a feature. Support voice is a brand asset, and it erodes quietly when every rep writes from scratch. The assistant retrieves the team's own best past answers and drafts in that established voice, so the output is grounded in what actually worked before — not a generic model's guess.\n\nBuilt on RAG over historical Zoho Desk tickets with embeddings for retrieval and exposed through MCP so it plugs into the tools reps already use. The human-in-the-loop boundary is deliberate: it accelerates the draft, the rep owns the send.",
      gallery: [],
      metrics: [
